...
Setting up OIL on your pages is really easy. All you need to do is include our main Javascript file and an additional configuration snippet. OIL uses JS events to communicate with its host site, please the table below for details.

For better clarity we've put together some code snippets and an example that shows how to integrate OIL and listen to OIL's opt-later event.
