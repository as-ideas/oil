...
****
The website which is using OIL should support Cross Origin Resource Sharing (CORS) from the hub domain and include it via HTTPS. The OIL hub origin needs to be included in the CORS-origin. The same-origin policy is an important security concept implemented by web browsers to prevent Javascript code from making requests against a different origin (e.g., different domain) than the one from which it was served:

[caption="CORS Principle"]
image::images/CORS_principle.png[CORS Principle, source: https://developer.mozilla.org/en-US/docs/Web/HTTP/Access_control_CORS#Access-Control-Allow-Origin]
...
For more details about CORS see https://developer.mozilla.org/en-US/docs/Web/HTTP/Access_control_CORS#Access-Control-Allow-Origin[here].
