...
IMPORTANT: Your domain needs to be whitelisted in order to work with OIL (contact us to get whitelisted: OIL Support <oil-support@axelsproinger>)
...
For the action element to save the current setting there are three API endpoints:
...
| window.AS_OIL.triggerSoiOptIn() | will trigger the CPC SOI opt-in (can also result in an opt-out)
| window.AS_OIL.triggerPoiOptin() | will trigger the CPC POI opt-in (can also result in an opt-out)
|====
