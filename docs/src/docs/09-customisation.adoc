...
their name prefix `oil_`). If such an event was detected it is propagated to Google Analytics using its API call `ga`. If
you want to use another tracking service, this is the function you have to replace by the corresponding API call. If you are not interested
in propagation of all OIL events, you can easily change the code to select your events by name. See chapter <<OIL Events>> for the
complete list of events sent by OIL.
